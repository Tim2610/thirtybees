...
class NotificationCore
{
    ...
    public function getTypes()
    {
        ...
        foreach ($this->types as $type => $description) {
            $ret[] = [
                ...
                'showAllLink' => (isset($description['showAllLink'])
                    ? $description['showAllLink']
                    : $link->getAdminLink($description['controller'])
                )
            ];
        }
        return $ret;
    ...
    public function getLastSeenId($type)
    {
        if (is_null($this->lastSeenIds)) {
            ...
            $sql = (new DbQuery())
                ...
                ->where('type IN (' . $types . ')');
            $result = $employeeInfos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
            if (is_array($result)) {
        ...
        return isset($this->lastSeenIds[$type])
            ? $this->lastSeenIds[$type]
            : 0;
    }
    ...
    public function markAsRead($type, $lastId)
    {
        ...
        return Db::getInstance()->insert(
            ...
            [
                'id_employee' => (int)$this->employeeId,
                'type' => pSQL($type),
                'last_id' => $lastId
            ],
            false,
    ...
    protected function getNewOrders($lastId, $limit)
    {
        ...
        foreach ($result as $row) {
            ...
            $results[] = [
                ...
                'id' => $id,
                'total' => Tools::displayPrice((float) $row['total_paid'], (int) $row['id_currency'], false),
    ...
    {
        // validate $definition
        $required = ['getNotifications', 'renderer', 'controller', 'icon', 'header', 'emptyMessage', 'showAll'];
